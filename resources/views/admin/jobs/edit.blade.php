        ...
        <div class="card card-custom card-sticky" id="kt_page_sticky_card">
          <div class="card-header" style="">
            <div class="card-title">
              <h3 class="card-label">Job Edit Form
                <i class="mr-2"></i>
                </h3>
          ...
          <!--begin::Form-->
            {{ Form::model($user, [ 'method' => 'PATCH','route' => ['jobs.update', $user->id],'class'=>'form' ,"id"=>"client_add_form", 'enctype'=>'multipart/form-data'])}}
              @csrf
              <input type="hidden" name="pattern" id="patternVal" value="{{$user->pattern}}">
              ...
              <div class="row">
                  ...
                  <div class="col-md-4 form-group ">
                      <div class="form-group  {{ $errors->has('user') ? 'has-error' : '' }}">
                          ...
                          {{ Form::select('user',$users, $user->customer_id, ['class' => 'no-padding form-control col-lg-12','id'=>'shop']) }}
                  ...
                  <div class="col-md-5 form-group ">
                      <div class="form-group">
                          ...
                          <div class="radio-inline">
                              <label class="radio">
                                  <input type="radio" value="1" @if($user->service_type == 1)  checked @endif name="type"/>
                                  <span></span>
                                  Carry In
                              </label>
                              <label class="radio">
                                  <input type="radio" value="2" @if($user->service_type == 2)  checked @endif name="type"/>
                                  <span></span>
                                  Pick Up
                              </label>
                              <label class="radio">
                                  <input type="radio" value="3" @if($user->service_type == 3)  checked @endif name="type"/>
                                  <span></span>
                                  On Site
                              </label>
                              <label class="radio">
                                  <input type="radio" value="4" @if($user->service_type == 4)  checked @endif name="type"/>
                                  <span></span>
                                  Courier
                              </label>
                          </div>
              ...
              <div class="row" id="courier" @if($user->service_type != 4) style="display: none" @endif>
                  <div class="col-md-4 form-group ">
                  ...
                  <div class="form-group col-md-4">
                      ...
                      <div class="">
                          <input type="text" name="tracking_id" class="form-control" id="tracking_id" value="{{$user->tracking_id}}" >
                      </div>
                  </div>
              </div>
              <div class="row">
                      ...
                      <div class="form-group  {{ $errors->has('device') ? 'has-error' : '' }}">
                          ...
                          <div class="">
                              <select name="device" id="device" class="form-control">
                                  <option value=""  @if($user->device->type == "") selected="selected" @endif>Select Device</option>
                                  <option value="1"  @if($user->device->type == 1) selected="selected" @endif>Mobile</option>
                                  <option value="2" @if($user->device->type == 2) selected="selected" @endif>Laptop</option>
                              </select>
...
{{--                              {{ Form::select('brand',$brands, $user->brand_id, ['class' => 'no-padding form-control col-lg-12','id'=>'brand']) }}--}}
                  ...
                  <div class="col-md-4 form-group ">
                      <div class="form-group  {{ $errors->has('device_model') ? 'has-error' : '' }}">
                          ...
                          <div class="device_model">
                              {{ Form::select('device_model',$devices, $user->device_id, ['class' => 'no-padding form-control col-lg-12','id'=>'brand']) }}
                          </div>
                  ...
                  <div id="pre_repair_div" class="col-md-12">
                      @if($user->preRepairs)
                          <div class="row">
                          @foreach($user->preRepairs as $pre)
                              <div class="col-md-4 form-group ">
                                  <div class="form-group  ">
                                      <label >{{$pre->name}}</label>
                                      <div class="custom_radio_container">
                                          <div class="custom_radio_group">
                                              <input class="custom_radio_input" type="radio" name="pre_repair{{$loop->iteration}}" id="{{$pre->name}}_yes" value="Yes" @if ($pre->value == "Yes") checked @endif>
                                              <label class="custom_radio_label" for="{{$pre->name}}_yes">
                                                  <img alt="" src="{{asset("assets/yes.png")}}" width="14"/>
                                              </label>
                                          </div>
                                          <div class="custom_radio_group">
                                              <input class="custom_radio_input" type="radio" name="pre_repair{{$loop->iteration}}" id="{{$pre->name}}_no" value="No" @if ($pre->value == "No") checked @endif>
                                              <label class="custom_radio_label" for="{{$pre->name}}_no">
                                                  <img alt="" src="{{asset("assets/no.png")}}" width="14"/>
                                              </label>
                                          </div>
                                          <div class="custom_radio_group">
                                              <input class="custom_radio_input" type="radio" name="pre_repair{{$loop->iteration}}" id="{{$pre->name}}_na" value="N/A" @if ($pre->value == "N/A") checked @endif>
                                              <label class="custom_radio_label" for="{{$pre->name}}_na">N/A</label>
                                          </div>
                                      </div>
                                  </div>
                              </div>
                          ...
                          <div class="">
                              <select name="product_configuration[]" multiple="multiple" id="product_configuration" class="form-control product-config">
                                  ...
                                  @if($jobSetting)
                                      ...
                                      @endif
                                  @endif
                                  @if($user->product_configuration)
                                      @php $configs = explode(', ',$user->product_configuration); @endphp
                                      @foreach($configs as $config)
                          ...
                          <div class="">
                              <select name="problem_by_customer[]" multiple="multiple" id="problem_by_customer" class="form-control problem_by_customer">
                                  @if($jobSetting)
                                      ...
                                      @endif
                                  @endif
                                      @if($user->problem_by_customer)
                                          @php $configs = explode(', ',$user->problem_by_customer); @endphp
                                          @foreach($configs as $config)
                                              <option value="{{ $config }}" selected>{{ $config }}</option>
                                          @endforeach
                                      @endif
                          ...
                          <div class="">
                              <select name="condition_of_product[]" multiple="multiple" id="condition_of_product" class="form-control condition_of_product">
                                  @if($jobSetting)
                                      ...
                                      @endif
                                  @endif
                                      @if($user->condition_of_product)
                                          @php $configs = explode(', ',$user->condition_of_product); @endphp
                                          @foreach($configs as $config)
                                              <option value="{{ $config }}" selected>{{ $config }}</option>
                                          @endforeach
                                      @endif
                  ...
                  <div class="col-md-4 form-group ">
                      <div class="form-group  {{ $errors->has('expected_delivery') ? 'has-error' : '' }}">
                          ...
                          <div class="">
                              {{ Form::date('expected_delivery', $user->expected_delivery, ['class' => 'form-control req form-control-solid','id'=>'name','placeholder'=>'Enter Here','required'=>'true']) }}
                  ...
                  <div class="col-md-4 form-group ">
                      <div class="form-group  {{ $errors->has('document') ? 'has-error' : '' }}">
                          ...
                          <div class="">
                              ...
                              @if($user->document)
                                  <a href="{{asset("uploads/$user->document")}}" target="_blank">{{$user->document}}</a>
                              @endif
                          </div>
                      </div>
                  </div>
                  ...
                  <div class="col-md-6 form-group ">
                      <div class="form-group  {{ $errors->has('id_card')? 'has-error' : '' }}">
                          ...
                          <div class="">
                              {{ Form::select('id_card',$idCards, $user->id_card_id, ['class' => 'no-padding form-control col-lg-12',]) }}
                      ...
                      <div class="row" >
                          ...
                          <div class="form-group   col-md-6">
                              ...
                              <div class="">
                             <span class="switch switch-outline switch-icon switch-success">
                              <label><input type="checkbox" checked="checked" name="profile" value="1">
                                <span></span>
                              </label>
                            </span>
                              </div>
                      ...
                      <table class="table table-striped table-bordered table-hover">
                          ...
                          @foreach($user->cards as $card)
                              <tr>
                                  ...
                                  <td><a href="{{route("remove-id-card",$card->id)}}"><i class="fa fa-times-circle btn-outline-danger"></i></a></td>
                              </tr>
                          @endforeach
                      </table>
                  </div>
    ...
    <script !src="">
        ...
        @if($user->pattern)
            $(document).ready(function() {
                var lock=new PatternLock('#pattern1',{setPattern : true});
                lock.setPattern("{{$user->pattern}}");
            });
        @endif
