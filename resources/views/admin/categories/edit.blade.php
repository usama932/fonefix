    ...
    <div class="d-flex flex-column-fluid">
      ...
      <div class="container">
        ...
        <div class="card card-custom card-sticky" id="kt_page_sticky_card">
          ...
          <!--begin::Form-->
            {{ Form::model($user, [ 'method' => 'PATCH','route' => ['categories.update', $user->id],'class'=>'form' ,"id"=>"client_update_form", 'enctype'=>'multipart/form-data'])}}
              @csrf
              <div class="row">
                  ...
                  <div class="my-5">
                    ...
                    @endif
                      <div class="form-group row {{ $errors->has('category') ? 'has-error' : '' }}">
                          ...
                          <div class="col-9">
                              {{ Form::select('category',$categories, $user->parent_id, ['class' => 'no-padding countries form-control col-lg-12','id'=>'device']) }}
...
@section('scripts')
    <script>
        $('.countries').prepend('<option selected=" ">Select a Category</option>');
        $(document).ready(function() {
            @if($user->parent_id)
                $('.countries').val("{{ $user->parent_id }}");
            @endif
        })
    </script>
@endsection
