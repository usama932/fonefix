...
<div class="card card-custom">
   <div class="card-header">
      ...
      <div class="card-toolbar">
         ...
         $add = $user->permission->user_add;
         }
         @endphp
         @if($add)
         <a href="{{ route('clients.create') }}" class="btn btn-primary font-weight-bolder mr-2">
         ...
         @endif
         <div class="btn-group">
            <a href="{{ route('client-import') }}"  class="btn btn-success font-weight-bolder">
            <i class="fa fa-file-import icon-sm"></i>Import</a>
            <a href="{{ route('client-export') }}"  id="kt_btn" class="btn btn-primary font-weight-bolder">
...
<script>
   ...
   var clients = $('#clients').DataTable({
       ...
       "columns": [
           ...
           {"data": "email"},
           {"data": "active"},
           {"data": "pending", "searchable": false, "orderable": false},
               @if($user->role == 1)
                   {
   ...
   function view_amount(id) {
   
    var CSRF_TOKEN = '{{ csrf_token() }}';
    $.post("{{ route('admin.getamount') }}", {_token: CSRF_TOKEN, id: id}).done(function (response) {
        $('.modal-body').html(response);
        $('#payModel').modal('show');
